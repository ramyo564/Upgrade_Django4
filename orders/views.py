...
def kakao_pay_approval(request):
    ...
    order = Order.objects.get(
        user=request.user, is_ordered=False, order_number=last_order_number
    )
    payment = Payment(
        user=request.user,
        payment_id=res["tid"],
    ...
    try:
        print(f"order_number = {last_order_number}")
        order = Order.objects.get(order_number=last_order_number, is_ordered=True)
