...
from django.db.models import Avg, Case, FloatField, Q, Value, When
...
def store(request, category_slug=None):
    ...
    if category_slug != None:
        ...
        for i in id:
            ...
            for variation_key in variation_category:
                items = product_test.variation_set.filter(
                    variation_category=variation_key
                )

                options_key_value.append(
                    list(items.values("variation_category", "variation_value"))
                )
    ...
    my_dict = {"colors": option_colors, "sizes": option_sizes}
...
def update_results(request):
    ...
    if request.method == "POST":
        ...
        if "category" in previous_url:
            ...
            products = (
                Product.objects.all()
                .filter(is_available=True, category__slug=category_name)
                .distinct()
            )
            ...
            if len(value_options) > 0:
                ...
                avg_review = (
                    products.annotate(avg_review=Avg("reviewrating__rating"))
                    .annotate(
        ...
        # 옵션 선택한 상태에서 한 번 더 선택할 때 -> 주소창은 test 옵션 존재할 때
            ...
            products = Product.objects.all().filter(
                is_available=True, category__slug=category_name
            )
        ...
        # 카테고리 옵션은 존재하지 않지만 url 은 여전히 test 일 때
            ...
            "update_results" in previous_url
            and len(value_options) == 0
            and request.session.has_key("category_name")
        ):
            print(3)
